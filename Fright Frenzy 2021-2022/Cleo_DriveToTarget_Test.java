...
@TeleOp(name="Cleo Drive To Target", group = "Test")
//@Disabled
public class Cleo_DriveToTarget_Test extends LinearOpMode
{
    Tank8271HardwareSetup robot = new Tank8271HardwareSetup();

    // Adjust these numbers to suit your robot.
    final float DESIRED_DISTANCE = 8.0f; //  this is how close the camera should get to the target (inches)
                                         //  The GAIN constants set the relationship between the measured position error,
                                         //  and how much power is applied to the drive motors.  Drive = Error * Gain
                                         //  Make these values smaller for smoother control.
    final float SPEED_GAIN =   0.02f ;   //  Speed Control "Gain". eg: Ramp up to 50% power at a 25 inch error.   (0.50 / 25.0)
    final float TURN_GAIN  =   0.01f ;   //  Turn Control "Gain".  eg: Ramp up to 25% power at a 25 degree error. (0.25 / 25.0)

    final float MM_PER_INCH = 25.40f ;   //  Metric conversion
    ...
    @Override public void runOpMode()
    {
        ...
        float  targetRange     = 0f;        // Distance from camera to target in Inches
        float  targetBearing   = 0f;        // Robot Heading, relative to target.  Positive degrees means target is to the right.
        float  drive           = 0f;        // Desired forward power (-1 to +1)
        float  turn            = 0f;        // Desired turning power (-1 to +1)
        ...
        while (opModeIsActive())
        {
            ...
            for (VuforiaTrackable trackable : targetsFreightFrenzy)
            {
                if (((VuforiaTrackableDefaultListener) trackable.getListener()).isVisible())
                {
                    ...
                    if (targetPose != null)
                    {
                        ...
                        float targetX = trans.get(0) / MM_PER_INCH; // Image X axis
                        float targetY = trans.get(2) / MM_PER_INCH; // Image Z axis
            ...
            if (gamepad1.left_bumper && targetFound) {
                ...
                float  headingError = targetBearing;
                ...
                drive = (float) (-gamepad1.left_stick_y  / 2.0);  // Reduce drive rate to 50%.
                turn  = (float) (gamepad1.right_stick_x / 4.0);  // Reduce turn rate to 25%.
                telemetry.addData("Manual","Drive %5.2f, Turn %5.2f", drive, turn);
            }
            telemetry.update();

            // Calculate left and right wheel powers and send to them to the motors.
            float leftPower    = Range.clip(drive + turn, -.6f, .6f) ;
            float rightPower   = Range.clip(drive - turn, -.6f, .6f) ;
